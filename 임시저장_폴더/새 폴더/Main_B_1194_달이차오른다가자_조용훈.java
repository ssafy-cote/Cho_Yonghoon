...
public class Main_B_1194_달이차오른다가자_조용훈 {
	static int n, m;
	static String[][] map;
						...
						if (!doorCheck(nx, ny)) {
							if (mapKeyCheck(nx, ny)) {
								if (keyCheck(tempPos, map[nx][ny])) {
									...
									System.out.println(nx + " :: " + ny + " 로 움직임");
	...
	static boolean mapKeyCheck(int x, int y) {
		return (map[x][y].equals("a") || map[x][y].equals("b") || map[x][y].equals("c") || map[x][y].equals("d")
				|| map[x][y].equals("e") || map[x][y].equals("f"));
	}
