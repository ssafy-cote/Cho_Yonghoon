 ...
 *
 */
public class Solution_2115_벌꿀채취_연민호_재귀조합_최적화_부분집합미리 {
	static int N;   //벌통의 크기
	...
	private static void combination(int cnt, int start) {
		//step 02. 두 일꾼의 벌통 선택 완료
		if(cnt==2) {

			int[] first = selectedIndex[0];		//첫번째 뽑은 꿀통 그룹의 시작 좌표배열
			int[] second = selectedIndex[1];	//두번째 뽑은 꿀통 그룹의 시작 좌표배열

			//step 03. 구한 수익이 최댓값이라면 갱신
			maxRevenue = Math.max( maxRevenue, maxSubset[first[0]][first[1]] + maxSubset[second[0]][second[1]] );
