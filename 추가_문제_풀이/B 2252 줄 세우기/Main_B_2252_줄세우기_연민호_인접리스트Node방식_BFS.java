 ...
 * 4   O
 * 
 * 메모리:41236KB, 시간:380ms
 * 
 * @author SSAFY
 *
 */
public class Main_B_2252_줄세우기_연민호_인접리스트Node방식_BFS {
	...
	public static void main(String[] args) throws IOException {
		...
		degrees = new int[N+1];	//정점의 차수 정보
		for(int i=0; i<M; i++) {
			st = new StringTokenizer(br.readLine());
			
			int from = Integer.parseInt(st.nextToken()); 	//학생 수
			int to = Integer.parseInt(st.nextToken());	//키 순서 정보
	...
	private static void BFS() {
		...
		while(!q.isEmpty()) {
			...
			for(Node to=adjList[from]; to!=null; to=to.next) {
				...
				if(--degrees[to.v]==0) q.offer(to.v);
			}
		}
	}
