...
/**
 * 
 * 메모리:12504KB, 시간:852ms
 * 
 * @author SSAFY
 *
 */
public class Main_B_1987_알파벳_연민호_배열1 {
	...
	//알파벳 사용 여부 체크를 위해
	//알파벳의 개수는 26개이므로 isUsed[0] : true라면 A 사용 중
	...
	public static void main(String[] args) throws IOException{
		...
		isUsed[board[0][0]]=true;	//시작 위치에 대한 알파벳 체크
		visit(0, 0, 1);
	...
	private static void visit(int r, int c, int cnt) {
		...
		for(int d=0; d<4; d++) {
			...
			if( isUsed[ board[nr][nc] ] ) continue;			//사용한 알파벳
			
			isUsed[ board[nr][nc] ] = true;	//알파벳 사용 체크
			visit(nr, nc, cnt+1);				//다음 좌표 방문
			isUsed[ board[nr][nc] ] = false;	//현재 위치 알파벳 사용 여부 되돌리기
		}
	}
}